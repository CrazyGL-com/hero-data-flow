...
export default function FlowStage(props) {
    const { rootRef, size, input, seed, reducedMotion, screenshot, screenAspect, screenScale, screenTilt, screenBrightness, screenshotX, screenshotY, particleCount, particleColor, accentColor, flowSpeed, particleSize, emissionRate, showLines, lineOpacity, lineMaxDistance, cursorAttract, cursorBurst, ambientStars, } = props;
    ...
    const pointsGeoRef = React.useRef(null);
    const pointsMatRef = React.useRef(null);
    const linesRef = React.useRef(null);
    const linesGeoRef = React.useRef(null);
    const linesMatRef = React.useRef(null);
    const starsRef = React.useRef(null);
    // Particle pool — allocated ONCE at mount, reused for the lifetime of
    // the component. No per-frame allocations.
    const posRef = React.useRef(null); // x,y,z per particle
    const velRef = React.useRef(null); // vx,vy,vz per particle
    const ageRef = React.useRef(null); // current age (s)
    const lifeRef = React.useRef(null); // total lifespan (s)
    const originRef = React.useRef(null); // ox,oy,oz per particle
    const colorRef = React.useRef(null); // r,g,b per particle (color attribute)
    const lineColorRef = React.useRef(null); // r,g,b,a × 2 verts per particle, but Three's LineBasicMaterial doesn't support per-vertex alpha — we encode alpha into RGB scale instead.
    const linePosRef = React.useRef(null); // x,y,z × 2 per particle
    ...
    const anchorWorldRef = React.useRef(null);
    ...
    const emitAccumRef = React.useRef(null);
    ...
    React.useEffect(() => {
        ...
        if (m)
            m.emissiveIntensity = screenBrightness;
    }, [screenBrightness]);
    // ─────────────────────────────────────────────────────────────────────
    ...
    useHeroAnimationFrame(rootRef, ({ delta }) => {
        ...
        if (!renderer || !scene || !camera || !positions || !velocities || !ages || !lifes || !origins || !colors || !linePositions || !lineColors || !hashes || !pGeom || !lGeom || !anchorWorld || !emitAccum)
            return;
        ...
        if (cursorActive) {
            ...
            cursorRayMatRef.current.set(input.x * 2 - 1, -(input.y * 2 - 1));
    ...
    return (_jsx("canvas", { ref: canvasRef, className: "crazygl-df-canvas", "aria-hidden": "true" }));
}
